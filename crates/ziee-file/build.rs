//! `ziee-file` build script — provisions the **file-only** build DB for sqlx
//! compile-time `query!` verification.
//!
//! ziee-file is the ONE schema-bound SDK crate (framework/core/identity are
//! build-DB-free). Its `query!` macros only touch the auth tables, so it needs
//! an file-only database — a sibling DB `ziee_file_build_<key>` on the same
...
//!
//! Migrations applied here are EXACTLY ziee-file's own `migrations/` dir (the
...
/// True when we should auto-provision our own file-only DB: no `DATABASE_URL`,
...
async fn main() {
    ...
    let database_url = if should_auto_isolate(&explicit) {
        ...
        let db_name = format!("ziee_file_build_{}", fnv1a_key(&manifest_dir));
            ...
            .await
        {
            Ok(admin) => {
                ...
                admin.close().await;
                println!("cargo:warning=ziee-file: file-only build DB → {db_name}");
                with_database(&base, &db_name)
            }
            Err(e) => {
                eprintln!("ziee-file build.rs: file-only DB provisioning skipped: {e}");
                base
            }
        }
    } else {
        ...
        .await
    {
        Ok(p) => p,
        Err(e) => {
            eprintln!("\nziee-file build.rs: failed to connect to build DB: {e}");
            panic!("ziee-file build DB connection failed");
        }
    };

    // Wipe + reapply the file migration subset (idempotent, isolated schema).
    ...
    sqlx::query("CREATE SCHEMA public")
        .execute(&pool)
        .await
        .expect("ziee-file build.rs: create schema failed");
    ...
    let migrator = sqlx::migrate::Migrator::new(migrations_path.clone())
        .await
        .unwrap_or_else(|e| panic!("ziee-file build.rs: migrator create failed: {e}"));
    if let Err(e) = migrator.run(&pool).await {
        eprintln!("\nziee-file build.rs: file migration apply failed: {e}");
        panic!("ziee-file migration failed");
    }
    pool.close().await;
