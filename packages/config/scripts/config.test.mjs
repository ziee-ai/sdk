...
// ── The generated stamp must name the sheets the run actually read ──────────

test('the stamp names the layered sheets, and is unchanged for the single-sheet shape', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'design-spec-stamp-'))
  const pkgTokens = path.join(dir, 'tokens.css')
  const appSheet = path.join(dir, 'app.css')
  fs.writeFileSync(
    pkgTokens,
    '@theme inline {\n  --color-background: var(--background);\n  --radius-lg: var(--radius);\n}\n' +
      ':root {\n  --background: #fff;\n  --radius: 0.5rem;\n}\n',
  )
  fs.writeFileSync(appSheet, '.dark {\n  --background: #000;\n}\n')

  const out = path.join(dir, 'OUT.md')
  const run = (args) =>
    spawnSync(process.execPath, [designSpec, ...args, '--out', out], { cwd: dir, encoding: 'utf8' })

  // Layered: the header names BOTH sheets. It used to hardcode
  // `src-app/ui/src/index.css`, so a layered app committed a generated doc
  // asserting a path it was not generated from.
  const layered = run(['--css', 'tokens.css', '--css', 'app.css'])
  assert.equal(layered.status, 0, layered.stderr)
  const header = fs.readFileSync(out, 'utf8').split('\n')[2]
  assert.match(header, /`tokens\.css` \+ `app\.css`/)
  assert.doesNotMatch(header, /src-app\/ui\/src\/index\.css/)

  // The ziee-shaped single-sheet run keeps the historical wording BYTE-for-byte:
  // its consumers have that string in a committed spec, and a cosmetic change
  // there turns `check:design-spec` red for a doc nobody edited.
  const single =
    fs.readFileSync(pkgTokens, 'utf8') + fs.readFileSync(appSheet, 'utf8')
  fs.mkdirSync(path.join(dir, 'src'), { recursive: true })
  fs.writeFileSync(path.join(dir, 'src', 'index.css'), single)
  for (const args of [[], ['--css', 'src/index.css']]) {
