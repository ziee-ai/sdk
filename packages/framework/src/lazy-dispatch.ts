...
// an in-flight stream.
//
// ── The one case retried BEFORE it can be poisoned: offline (issue #161) ────
// Being offline is not a hard 404, but it produces one if `importModule()` is
// ever called while offline: the fetch fails, the module map caches that
// failure against the specifier forever, and — unlike a deploy, where the
// specifier itself changes on the next visit — the SAME chunk stays broken
// after reconnecting, for the life of the document, until a full reload. That
// was FUP-65: an on-demand click made offline stayed broken after reconnect.
// The fix is below `importWithRetry`: check `isOffline()` BEFORE calling
// `importModule()` and fail fast without calling it at all when offline, so
// the specifier is never touched while it cannot succeed. The next dispatch —
// a later click, or a warm resumed on `online` (`./chunk-recovery`'s
// `warmUnlessOffline`, used by `store-kit.ts` and the shell's
// `usePrefetchModules`) — then gets a genuinely fresh, unpoisoned fetch.
//
...
export function createLazyDispatcher<M = any>(
  ...
  const importWithRetry = async (): Promise<M> => {
    ...
    for (let attempt = 0; attempt <= MAX_IMPORT_RETRIES; attempt++) {
      // ITEM-5b (issue #161 / FUP-65): never call `importModule()` while
      // offline. Per the HONEST LIMIT above, a failed `import()` permanently
      // poisons that specifier's module-map entry for the life of the
      // document — attempting it while offline would therefore burn the
      // chunk this dispatcher exists to recover, and no later retry (online
      // or not) could re-fetch it without a full page reload. Fail fast
      // instead, WITHOUT ever touching `importModule()`, so the specifier is
      // never poisoned: the NEXT dispatch (a later click, or store-kit's /
      // usePrefetchModules' idle warm resuming on `online` — see
      // ./chunk-recovery's `warmUnlessOffline`) gets a genuinely fresh,
      // unpoisoned fetch. This is the "online-aware retry" — never a
      // cache-busted re-import of the SAME url, which is not available here:
      // the specifier is generated by the bundler and is not ours to alter.
      if (isOffline()) {
        lastError = new Error(
