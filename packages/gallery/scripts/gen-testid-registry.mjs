 ...
 * literal across the CONFIGURED source trees, for agentic + i18n-safe testing.
 *
 ...
 * roots + the output path all come from `resolveRegistryScope(cwd)`;
 * `resolveGalleryConfig(cwd)` supplies:
 ...
 *   - output        = `testidOut`.
 *
 * THE PARTITION: where `testidOut` sits decides what is walked.
 *   - `testidOut` OUTSIDE every `kitTestIds` root → the APP'S OWN registry (the
 *     generator's default `<srcDir>/components/ui/testIds.generated.ts`, or the
 *     config's explicit app-local path). It holds the app UNION: app trees ∪
 *     package trees.
 *   - `testidOut` INSIDE a `kitTestIds` root (`isKitSurface`) → the KIT'S OWN
 *     COMMITTED SHARED surface (`@ziee/kit/testIds.generated`, the import surface
 *     every consumer shares). It must hold ONLY the package trees — one consumer
 *     app's ids in the shared surface would make every OTHER consumer stale by
 *     construction, no matter how often it regenerates. Each app's union belongs
 *     in the app's own registry instead.
 * Both scopes render with the SAME neutral header descriptor, so the generated
...
/**
 * Resolve the full generation scope from the gallery config at `cwd` (a string,
 * default `process.cwd()`) — or from an explicitly RESOLVED config object (the
 * shape `resolveGalleryConfig` returns: `{ __cwd, srcDir, extraTrees,
 * kitTestIds, testidOut, … }`), so a test can build a scope from literal paths
 * without a config file. This is the ONE scope-resolution implementation; both
 * `main()` and the golden tests use it, so they can never diverge.
 *
 * Returns:
 *   - `cfg`        — the resolved config (same object that was passed, when one was);
 ...
 */
export function resolveRegistryScope(cwd = process.cwd()) {
  const cfg = typeof cwd === 'string' ? resolveGalleryConfig(cwd) : cwd
  const base = cfg.__cwd ?? process.cwd()
  ...
  const walks = isKitSurface ? pkgTrees : [...appTrees, ...pkgTrees]
  const files = walks.flatMap((r) => collectSourceFiles(r))
  return { cfg, appTrees, pkgTrees, files, out, isKitSurface }
}
