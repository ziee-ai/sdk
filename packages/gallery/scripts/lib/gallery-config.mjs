...
const DEFAULTS = {
   ...
   * testids are the source of truth for kit-component ids. The partition: a
   * `testidOut` INSIDE a `kitTestIds` root is the kit's own shared surface
   * (kit/shell ids ONLY); ANY OTHER `testidOut` is the app's own registry
   * (app ∪ kit/shell). `[]` = a self-contained app whose kit ids still live
   * under `srcDir` (the pre-package layout). */
  kitTestIds: [],
  /**
   * Output path (relative to cwd) for the generated testid registry. The
   * partition decides what it holds: a `testidOut` INSIDE a `kitTestIds` root is
   * the kit's own shared surface (`@ziee/kit/testIds.generated` — kit/shell ids
   * ONLY, the import surface every consumer shares); ANY OTHER `testidOut` is
   * the app's own registry (app ∪ kit/shell). `null` = the historical in-app
   * default `<srcDir>/components/ui/testIds.generated.ts`. */
  testidOut: null,
