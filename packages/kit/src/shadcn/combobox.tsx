    ...
    <ComboboxPrimitive.Portal container={portalContainer}>
     <PortalContainerInherit>
      <ComboboxPrimitive.Positioner
        ...
        // pointer-events-auto is the critical bit (same as the Select): our Drawer
        // is a modal Radix Dialog that sets `body { pointer-events: none }`, which
        // the body-portaled popup would otherwise inherit — making its options
        // impossible to click.
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-[calc(--spacing(5.25))] w-fit items-center justify-center gap-1 rounded-sm bg-muted px-1.5 text-xs font-medium whitespace-nowrap text-foreground has-disabled:cursor-not-allowed has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0",
        className
      )}
      {...props}
    >
      ...
      {showRemove && (
        <ComboboxPrimitive.ChipRemove
          ...
          className="-ml-1 opacity-50 enabled:hover:opacity-100"
